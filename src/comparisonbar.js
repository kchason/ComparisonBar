  ...
  $.fn.comparisonbar = function (options) {
    this.each(function () {
      ...
      let barFillPercentage =
        (settings.value - settings.minValue) /
      ...
      if (settings.showMinMax) {
        ...
        $(minLine)
          .attr({
            ...
            y1:
              Math.round((elementHeight - settings.barHeight) / 2) +
              settings.barHeight,
        ...
        $(maxLabel)
          .attr({
            x:
              Math.round(0.95 * elementWidth) -
              settings.maxValue.toString().length * 5,
            y:
              Math.round((elementHeight - settings.barHeight) / 2) +
              settings.barHeight +
              17
      ...
      $.each(settings.zones, function (key, value) {
        ...
        let startX =
          (zoneSettings.min - settings.minValue) /
          (settings.maxValue - settings.minValue)
        let endX =
          (zoneSettings.max - settings.minValue) /
          (settings.maxValue - settings.minValue)
        ...
        $(zoneLine)
          .attr({
            x1:
              Math.round(startX * 0.9 * elementWidth) +
              Math.round(0.05 * elementWidth),
            y1: zoneY,
            x2:
              Math.round(endX * 0.9 * elementWidth) +
              Math.round(0.05 * elementWidth),
            y2: zoneY
          })
          .css({
      ...
      $.each(settings.markers, function (key, value) {
        ...
        if (markerSettings.value != null) {
          ...
          let markerPercentage =
            (markerSettings.value - settings.minValue) /
            (settings.maxValue - settings.minValue)
          // Ensure marker percentage is within 0 - 100
          if (markerPercentage < 0) {
            markerPercentage = 0
          }
          if (markerPercentage > 1) {
            markerPercentage = 1
          }
          // Build label and marker objects
          const labelX = Math.round(
            markerPercentage * 0.9 * elementWidth +
              0.05 * elementWidth -
              markerSettings.label.length * 3
          )
          // Ensure label doesn't go off screen
          if (labelX < 0) {
            markerPercentage = 0
          }
          if (labelX + markerSettings.label.length * 4 > elementWidth) {
            markerPercentage = elementWidth - markerSettings.label.length * 4
          }
          ...
          if (markerSettings.position === 'top') {
            points = ''.concat(
              Math.round(markerPercentage * 0.9 * elementWidth) -
                10 +
                0.05 * elementWidth,
              ',',
              Math.round((elementHeight - settings.barHeight) / 2) - 10,
              ' ',
              Math.round(markerPercentage * 0.9 * elementWidth) +
                0.05 * elementWidth,
              ',',
              Math.round((elementHeight - settings.barHeight) / 2) + 5,
              ' ',
              Math.round(markerPercentage * 0.9 * elementWidth) +
                10 +
                0.05 * elementWidth,
