(function ( $ ) {
	...
	$.fn.comparisonbar = function(options){
		...
		this.each(function() {
			...
			//FILL BAR
			var barFillPercentage = ((settings.value - settings.minValue) / (settings.maxValue - settings.minValue));
			...
			//Create object
			var barFill = document.createElementNS(namespace, "rect");
			$(barFill).attr({
				x: Math.round(0.05 * elementWidth),
				y: Math.round((elementHeight-settings.barHeight) / 2),
				height: settings.barHeight,
			...
			if(settings.showMinMax){
				...
				var minLine = document.createElementNS(namespace, "line");
				var maxLine = document.createElementNS(namespace, "line");
				...
				//Add text
				var minLabel = document.createElementNS(namespace, "text");
				var maxLabel = document.createElementNS(namespace, "text");
			...
			$.each(settings.zones, function(key, value) {
				...
				if(zoneSettings['position'] == 'top'){
					var zoneY = Math.round((elementHeight-settings.barHeight) / 2);
				}else{
					var zoneY = Math.round((elementHeight-settings.barHeight) / 2)+settings.barHeight;
				}
				
				//Calculate start and stop
				var startX = ((zoneSettings['min']-settings.minValue) / (settings.maxValue - settings.minValue));
				var endX = ((zoneSettings['max'] - settings.minValue) / (settings.maxValue - settings.minValue));
				...
				if(endX > 1){ endX = 1; }
				//Create line
				var zoneLine = document.createElementNS(namespace, "line");
